...
public:
    ...
    const unsigned char& at(int x, int y) const { return square[y][x]; }
    int turn() { return turn_; }
    bool is_violet() const { return (turn_ & 1) == 0; }
    ...
    int evaluate() const { return eval_blocks() + eval_effect(); }
    int nega_eval() const { return is_violet() ? evaluate() : -evaluate(); }
    int violet_score() const { return calc_score(block_info); }
    int orange_score() const { return calc_score(block_info + NBLOCK); }
    int nega_score() {
...
protected:
    ...
    int calc_score(const unsigned char* blks_vec) const;
    int eval_blocks() const;
